            ...
            <form method="POST" action="{{ route('admin.instituicoes.store') }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="codigo_uo" id="codigo_uo" value="{{ old('codigo_uo') }}" required pattern="\d{4,6}" maxlength="6"
                    ...
                    <div>
                        ...
                        <select name="tipo" id="tipo" required class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                            ...
                            <option value="Ministério" {{ old('tipo') == 'Ministério' ? 'selected' : '' }}>Ministério</option>
                            <option value="Governo Provincial" {{ old('tipo') == 'Governo Provincial' ? 'selected' : '' }}>Governo Provincial</option>
                            <option value="Administração Municipal" {{ old('tipo') == 'Administração Municipal' ? 'selected' : '' }}>Administração Municipal</option>
                            <option value="Instituto Público" {{ old('tipo') == 'Instituto Público' ? 'selected' : '' }}>Instituto Público</option>
                            <option value="Empresa Pública" {{ old('tipo') == 'Empresa Pública' ? 'selected' : '' }}>Empresa Pública</option>
                        </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="responsavel" id="responsavel" value="{{ old('responsavel') }}" required
                           class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg shadow-sm">Criar Instituição</button>
                </div>
            </form>
        </div>
    </div>
